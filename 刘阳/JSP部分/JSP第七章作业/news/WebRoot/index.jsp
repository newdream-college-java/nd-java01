...
<div id="header">
	<div id="top_login">
		...
		<%}else{%>
		欢迎你，<%=(String)session.getAttribute("uname")%> | <a href="admin.jsp">登录控制台</a> | <a href="deleteSession.jsp" target="_self" >登出</a>
		<%}%>
  </div>
  <div id="nav">
  ...
  <div class="main">
    ...
    <div class="content">
      ...
      <ul class="classlist">
    		...
    		for(News news:list){
    		 %>
      		<li><a href="#"> <%=news.getUtitle() %> </a>
      		<span><%=news.getDate()%> </span></li> 	
      		<%}%>
      		...
      		<p align="right"> 当前页数:[<%=(Integer)request.getAttribute("cpage")%>/<%=((Integer)request.getAttribute("count")/(Integer)request.getAttribute("pageSize"))+1%>]&nbsp;&nbsp; 
       <a href="fenye.jsp?curPage=1">首页</a>
       <% 
       if((Integer)request.getAttribute("cpage")==1){
       %><a href="" disabled="true">&nbsp;&nbsp;上一页</a><%
       }else{%>
       <a href="fenye.jsp?curPage=<%=((Integer)request.getAttribute("cpage")-1 )%>">&nbsp;&nbsp;上一页</a><%} %>
       <% 
       if((Integer)request.getAttribute("cpage")==((Integer)request.getAttribute("count")/(Integer)request.getAttribute("pageSize"))+1){
       %><a href="" disabled="true">&nbsp;&nbsp;下一页</a><%
       }else{%>
       <a href="fenye.jsp?curPage=<%=((Integer)request.getAttribute("cpage")+1 )%>">&nbsp;&nbsp;下一页</a><%} %>
       <a href="fenye.jsp?curPage=<%=((Integer)request.getAttribute("count")/(Integer)request.getAttribute("pageSize"))+1%>">&nbsp;&nbsp;末页</a>
       </p>
