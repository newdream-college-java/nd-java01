...
<div id="admin_bar">
  <div id="status">管理员： 登录  &nbsp;&nbsp;&nbsp;&nbsp; <a href="deleteSession.jsp">退出</a></div>
  <div id="channel"> </div>
</div>
...
<div id="main">
  ...
  <div id="opt_area">
    ...
    <form action="news_add_control.jsp" method="post" onsubmit="return check()" enctype="multipart/form-data">
      <p>
        <label> 主题 </label>
        <select name="ntid" id="nid">
    		...
    		for(int i=0;i<alist.size();i++){
    		 %>
    		 <%if(i==0){%>
        	<option selected="selected" value=<%=i+1 %>><%=alist.get(i) %></option>
        	<%
    		}else{%> 
    		<option value=<%=i+1 %>><%=alist.get(i) %></option>   		  
			<%
    		}}
    		 %>  	 
        </select>
      ...
      <p>
        <label> 上传图片 </label>
        <input name="file" class="opt_input" type="file" style="height:25px"/>
      </p>
