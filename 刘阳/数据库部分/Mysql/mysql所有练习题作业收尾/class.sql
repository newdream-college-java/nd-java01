...
SELECT REPLACE('   爱你  要你  我  爱  你   ',' ','') -- 这个只能去除首部的空格

-- 练习二第14题：计算每个学员身份证中字符‘1’出现的次数
SELECT LENGTH(StuCard)-LENGTH(REPLACE(StuCard,'1','')) '1出现的次数' FROM studentinfo
...
-- 练习二第18题：查询所有学员入学时间，要求显示格式为‘2007年03月02日’
SELECT DATE_FORMAT(StuJoinTime,'%Y年%m月%d日') '入学时间' FROM studentinfo

-- 练习二第19题：查询当前时间，要求显示格式为‘22时57:37’
select date_format(now(),'%h时%i:%s');

-- 练习二第20题：查询2007年入学的学员信息
SELECT * FROM studentinfo WHERE StuJoinTime<'2007-12-31' AND StuJoinTime>'2007-01-01'
...
SELECT t.TeacherName '班主任名字',SUM(stue.ExamResult) '总成绩'FROM studentinfo stu,classinfo c,studentexam stue,teacherinfo t WHERE stu.SClassID=c.ClassID AND c.CTeacherID=t.TeacherID AND stu.StuID=stue.EStuID GROUP BY t.TeacherID;

-- 练习3第13题：统计每个月入学学员的平均成绩
SELECT avg(e.ExamResult) FROM studentexam e,studentinfo i WHERE e.estuID=i.stuID GROUP BY DATE_FORMAT(i.StuJoinTime,'%m')
