...
<div id="header">
	<div id="top_login">
		<c:if test="${empty uname}" var="namess">		
			<form action="index_control" method="post" onsubmit="return check()">
...
<div class="sidebar">
  ...
  <div class="main">
    ...
    <div class="content">
      <ul class="class_date">
      	<li id="class_month">
      		<c:forEach var="titleList" items="${alist}">
      			<a href="#"><b><c:out value="${titleList}"></c:out></b></a>   		
      		</c:forEach>
      </li></ul>
      <ul class="classlist">
      		<c:forEach var="newsList" items="${li}">
      			<li><a href="#"><c:out value="${newsList.getNtitle()}"></c:out></a>
      			<span><c:out value="${newsList.getNcreatdate()}"></c:out></span></li>     		
      		</c:forEach>     		    		
       </ul>
       <p align="right"> 当前页数:[${cpage}/${count}]&nbsp;&nbsp; 
       <c:choose> 
	        <c:when test="${cpage!=1}">
		       <a href="index_controls.jsp?curPage=1">首页</a>      
		       <a href="index_controls.jsp?curPage=${cpage-1}">&nbsp;&nbsp;上一页</a>
	 		</c:when>      
     		<c:otherwise>  
			<a href="#">首页</a>    
	        <a href="#">&nbsp;&nbsp;上一页</a>
		...
		<c:choose>
			<c:when test="${cpage!=count}">
       			<a href="index_controls.jsp?curPage=${cpage+1}">&nbsp;&nbsp;下一页</a>
       			<a href="index_controls.jsp?curPage=${count}">&nbsp;&nbsp;末页</a>
       		</c:when>
       		<c:otherwise>
        		<a href="#">&nbsp;&nbsp;下一页</a>
       			<a href="#">&nbsp;&nbsp;末页</a>
