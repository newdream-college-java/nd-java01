...
<div id="header">
	<div id="top_login">
		...
		<%}else{%>
		欢迎你，<%=session.getAttribute("uname")%> | <a href="#">登录控制台</a> | <a href="loginOut.jsp">登出</a>
		<%}%>
  </div>
  <div id="nav">
    ...
    <div class="content">
      ...
      <ul class="classlist">
      		<%   
      			...
      			List<News> alist=(ArrayList<News>)request.getAttribute("list");
      			for(News news:alist){
    		 %>
      		<li><a href="#"> <%=news.getNauthor()%> </a>
      		<span><%=news.getNcreatedate()%> </span></li> 	
      		<%}%>
      		<p align="right"> 当前页数:[<%=(Integer)request.getAttribute("apg") %>|<%=(Integer)request.getAttribute("count") %>]
      		<a href="index_cont.jsp?apg=1">首页</a>
      		 <% 
      		 if((Integer)request.getAttribute("apg")==1){
       		%><a href="" disabled="true">&nbsp;&nbsp;上一页</a><%
      		 }else{%>
       		<a href="index_cont.jsp?apg=<%=((Integer)request.getAttribute("apg")-1 )%>">&nbsp;&nbsp;上一页</a><%} %>
      		<% 
      		 if((Integer)request.getAttribute("apg")==(Integer)request.getAttribute("count")){
       		%><a href="" disabled="true">&nbsp;&nbsp;下一页</a><%
      		 }else{%>
       		<a href="index_cont.jsp?apg=<%=((Integer)request.getAttribute("apg")+1 )%>">&nbsp;&nbsp;下一页</a><%} %>
      		<a href="index_cont.jsp?apg=<%=(Integer)request.getAttribute("count") %>">&nbsp;&nbsp;末页</a></p>
  ...
  <div class="friend_list">
	...
	<div style="margin-left:300px;">
    	<br/>本站在线人数：<span style="font-size: 20px;color:red;font-weight:bold;"><%
    		...
    		List loginedUsers=(ArrayList)application.getAttribute("uname");
    		out.print(loginedUsers.size());
