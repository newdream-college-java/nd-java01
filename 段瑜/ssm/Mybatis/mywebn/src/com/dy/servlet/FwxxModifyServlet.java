...
public class FwxxModifyServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(fwid!=null&&fwid.trim().length()>0){
			fw.setFwid(Integer.parseInt(fwid));
		}
		Calendar calendar = Calendar.getInstance();
		Date time = calendar.getTime();
		fw.setDate(time);
		int updateByPrimaryKey = mapper.updateByPrimaryKeySelective(fw);
		if(updateByPrimaryKey>0){
			out.println("<script>alert('�޸ĳɹ�');location.href='FwxxListServlet'</script>");
			sqlSession.commit();
		}else{
			sqlSession.rollback();
			out.println("<script>alert('�޸�ʧ��');history.go(-1)</script>");
		}
	}
