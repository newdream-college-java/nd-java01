...
insert into DEPT values(DEPT_ID_SEQ.nextval,'WORD','WORD');
...
male char(3)
	...
	insert into st values(1,'TOM',21,'男');
	insert into st values(2,'kite',22,'男');
	 insert into st values(3,'john',23,'女');

create index ind1 on st(sno);  

-- 索引的全部信息 select * from user_indexes; 
select * from user_indexes;
--查询索引涉及到的列 select * from user_ind_columns where index_name='IND1’;
select * from user_ind_columns where index_name='IND1';
